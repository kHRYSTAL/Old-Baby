package com.oldbaby.oblib.util.luban;
...
import static com.oldbaby.oblib.util.luban.Preconditions.checkNotNull;
 ...
 */
public class Luban {
    private volatile static Luban instance;
    ...
    public static Luban getInstance(Context context) {
        if (instance == null) {
            synchronized (Luban.class) {
                if (instance == null)
                    instance = new Luban(Luban.getPhotoCacheDir(context.getApplicationContext()));
            }
        }
        return instance;
    }
    ...
    public Bitmap compressReturnBitmap(Bitmap bitmap) {
