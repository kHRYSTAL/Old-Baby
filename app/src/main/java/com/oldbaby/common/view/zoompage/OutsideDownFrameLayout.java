...
public class OutsideDownFrameLayout extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            if (getScrollY() <= 0 && ev.getY() - downY > DensityUtil.dip2px(15)) {
                mLastY = ev.getY();
                return true;
            }
        }
    ...
    protected void process() {
        if (-getScrollY() > DRAG_Y_MAX) {
            ...
            int visiableHeight = rect.height() == 0 ? DensityUtil.getHeight() : rect.height();
            mMoveY = Math.abs(-visiableHeight - getScrollY());
            MLog.e(TAG, "visable height:" + rect.height());
            MLog.e(TAG, "moveY:" + mMoveY);
