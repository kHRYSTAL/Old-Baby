...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (mIsBeingDragged) {
            setScrollParentDisallowInterceptTouch(getParent(), true);
        }
    ...
    /**
     * 如果父view中有scrollView 在onInterceptTouchEvent返回true时，禁止scroll再去抢劫touch。然后在onTouchEvent将禁止解放。
     */
    private void setScrollParentDisallowInterceptTouch(ViewParent viewParent, boolean allow) {
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
            setScrollParentDisallowInterceptTouch(getParent(), false);
        }
    ...
    /**
     * 这行代码不是lib的.根据mMode重置mCurrentMode
     * 解决的问题是，手动上拉加载更多后，其他地方再调用该方法时，执行上拉加载更多
     */
    public final void resetCurrentMode() {
        mCurrentMode = (mMode != Mode.BOTH) ? mMode : Mode.PULL_FROM_START;  //这行代码不是lib的，后加的。解决的问题是，手动上拉加载更多后，其他地方再调用该方法时，执行上拉加载更多
    }
